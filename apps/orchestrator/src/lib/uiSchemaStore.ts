...
async function resolveImplicitSpecId(
	origin: string | null,
): Promise<string | null> {
	...
	if (origin?.trim()) {
		const fromBinding = await uiOriginsRepo.findSpecIdForOrigin(
			normalizeOrigin(origin),
		);
		if (fromBinding != null) return fromBinding;
		const fromDoc = await uiSpecsRepo.findSpecIdForOrigin(
			normalizeOrigin(origin),
		);
...
export async function setUiOverride(
	specId: string,
...
export async function setOriginBinding(
	origin: string,
	specId: string | null,
): Promise<void> {
